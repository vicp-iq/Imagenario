...
public class Imagenario {
	...
	static int[] PLAYER_PALETTE = { 0xffffff, 0x0000ff, 0xff0000, 0x00ff00, 0xffff00, 0x00ffff, 0xff00ff, 0x7f7f7f,
			0xff7f00, 0x000000 };
	...
	static SCX scx;
	static String destinationPath = "D:/SteamLibrary/SteamApps/common/Age2HD/resources/_common/scenario/text.scx";
	...
	/**
	 * palette : the palette filename for generating terrain tiles mode : 0-trim,
	 * 1-stretch plants : 0-no, 1-yes max_height : maximum height of hill, also the
	 * amount of levels of hill open_dict : dictionary of opening save_dict :
	 * dictionary of saving
	 */
	static HashMap<String, Object> settings = new HashMap<String, Object>();
	
	static public void main(String[] args) {
		final MainFrame mainframe = new MainFrame();
		colormap = new Palette(COLORMAP_ARTIST);
		drawing_colormap = new Palette(COLORMAP_DRAWING);

		// load map image
		srcfile = new File("Inputs/map.png");
		...
		try {
			...
			JOptionPane.showMessageDialog(mainframe, "File " + srcfile + " not found");
			return;
		}

		// load height map image
		srcfile2 = new File("height.png");

		try {
			BufferedImage img = ImageIO.read(srcfile2);
			srcimg2 = img;
		} catch (IOException e1) {
			JOptionPane.showMessageDialog(mainframe, "File " + srcfile2 + " not found");
			return;
		}
		...
		try {
			...
			JOptionPane.showMessageDialog(mainframe, "File " + RAW + " not found");
			return;
		}
		...
		ImageProcessor.setTerrainSizeByImage(terr, srcimg);
		scx.removeAllUnits();
		...
		SCXManager.save(destinationPath, scx);

		System.out.println(scx.players[0].unit_count);

		System.out.println(scx.player_count[0]);
		System.out.println(scx.player_count[1]);
		System.out.println(scx.player_count[2]);
	...
	static public void main2(String[] args) {
		...
		mainframe.button_open.addActionListener(e -> Open(mainframe));
		mainframe.button_open2.addActionListener(e -> Open2(mainframe));
		mainframe.button_process.addActionListener(e -> Process(mainframe));
		mainframe.button_save.addActionListener(e -> Save(mainframe));
	...
	static void Open(MainFrame mainframe) {
	...
	static void Open2(MainFrame mainframe) {
		if (mainframe.chooser1.showOpenDialog(mainframe) != JFileChooser.APPROVE_OPTION)
			return;
	...
	static void Process(MainFrame mainframe) {
		try {
			scx = SCXManager.load(RAW);
		} catch (FileNotFoundException exception) {
	...
	static void Save(MainFrame mainframe) {
		if (scx == null) {
